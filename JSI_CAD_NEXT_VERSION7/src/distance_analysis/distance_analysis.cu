...
float minimum_distance(const EvalAndConstructTask &t1, const EvalAndConstructTask &t2) {
  ...
  while (true) {
    ...
    auto find_or_add_unit = [](std::vector<DistRefineUnit> &units, const DistEndpoint &endpoint,
                               const EvalAndConstructTask &task) {
      for (size_t idx = 0; idx < units.size(); ++idx) {
        if (units[idx].endpoint.slot == endpoint.slot && units[idx].endpoint.bbox == endpoint.bbox) return idx;
      }
      units.push_back({endpoint, task});
      return units.size() - 1;
    };

    std::vector<DistRefineUnit> unique1;
    std::vector<DistRefineUnit> unique2;
    std::vector<DistReuseJob> reuse_jobs;
    unique1.reserve(jobs.size());
    unique2.reserve(jobs.size());
    ...
    for (const auto &job : jobs) {
      ...
      const auto &a1 = ch.side1[static_cast<size_t>(job.side1.slot)];
      const auto &a2 = ch.side2[static_cast<size_t>(job.side2.slot)];
      const size_t side1_unit =
          find_or_add_unit(unique1, job.side1,
                           make_dist_refine_task(t1, a1, job.side1.bbox, 1, global_nuv));
      const size_t side2_unit =
          find_or_add_unit(unique2, job.side2,
                           make_dist_refine_task(t2, a2, job.side2.bbox, 2, global_nuv));
