...
void EG_spline2dDeriv_irrational_threshold(int *ivec, float *data, float ustep, float vstep, float ustart,
                                           float vstart, int nuv, float *results, int omp_threshold) {
  CAD_PROFILE_SCOPE("EG_spline2dDeriv_irrational");
  EvalContext ctx = BuildEvalContext(ivec, data);
#pragma omp parallel for schedule(static) collapse(2) if (nuv * nuv > omp_threshold)
...
void EG_spline2dDeriv_irrational(int *ivec, float *data, float ustep, float vstep, float ustart, float vstart,
  ...
  EG_spline2dDeriv_irrational_threshold(ivec, data, ustep, vstep, ustart, vstart, nuv, results, 0);
}
...
#pragma omp parallel for schedule(static) collapse(2) if ((y1 - y0) * nuv > 0)
